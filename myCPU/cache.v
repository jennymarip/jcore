...
/*-------------------------------BLOCK RAM(v,tag)----------------------------------*/
reg [19:0] reg_tag;
assign addr_ok_addr = {32{valid}} & {tag, index, offset};
...
always @(posedge clk)
    begin
        if(~resetn)begin
            reg_tag <= 0;
        end else if((curr_state == IDLE && valid && wb_curr_state != WB_WRITE) || (curr_state == LOOKUP && next_state == LOOKUP))begin
            reg_tag <= tag;
        end
    end
...
assign rd_addr = {buff_tag, buff_index, 4'b0}; // miss情况需要读取整个cacheline，基地址是整个cacheline起始地址
